...
export default function App() {
  ...
  const [user, setUser] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("userInfoLogedIn"));
    } catch {
      return null;
    }
  });
  ...
  const TokoGuard = ({ id }) => {
    ...
    console.log("cek-user_loggedId ==> " + user);
  ...
  //     const res = await api.post(`${process.env.REACT_APP_API_URL}/auth/login`, { username, password });
  //     localStorage.setItem("tokenn", res.data.accessToken);
  //     // window
  ...
  const handleLogin2 = (userData) => { 
    console.log("Logged in as : "+ JSON.stringify(userData));
    setUser(userData);
  };
  console.log("Cekk-global0-userloggedin = "+ JSON.stringify(user));
  return (
    <>
      {!user ? (
        /* ===== Belum login ===== */
        <Routes>
          <Route
            path="/"
            element={<Login onLogin={handleLogin2} />} //users={users} 
          />
            ...
            <main className="flex-1 min-w-0 overflow-y-auto p-4">
              <Routes>
                ...
                <Route
                  ...
                  element={
                    <ProtectedRoute
                      allowedRoles={["SUPER_ADMIN", "PIC_TOKO"]}
                    >
                      <Dashboard user={user} />
                    </ProtectedRoute>
                  }
                ...
                <Route
                  ...
                  element={
                    <ProtectedRoute allowedRoles={["SUPER_ADMIN", "PIC_TOKO"]}>
                      <UserManagement />
                    </ProtectedRoute>
                  }
                />
