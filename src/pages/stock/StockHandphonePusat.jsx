// src/pages/stock/StockHandphonePusat.jsx
...
// Kunci item: imei (utama) → fallback nama
const makeKey = (row) => {
...
export default function StockHandphonePusat({ user }) {
  ...
  // Auto refresh jika ada transfer di tab lain
  useEffect(() => {
  ...
  function exportExcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "HP_PUSAT");
    const ymd = today().replace(/-/g, "");
    XLSX.writeFile(wb, `STOCK_HP_PUSAT_${ymd}.xlsx`);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold">Stok Handphone — PUSAT</h1>
          <p className="text-slate-600 text-sm">
            Base dari katalog + override local (PUSAT). Transfer ke toko via modal.
          </p>
        </div>
      ...
      {open && targetRow && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-lg p-4 w-full max-w-lg">
            <h3 className="text-lg font-semibold mb-3">Transfer Handphone dari PUSAT ke Toko</h3>
