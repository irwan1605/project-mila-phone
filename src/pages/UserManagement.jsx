...
export default function UserManagement() {
  ...
  const fetchUserList = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/users/getAllDatas`);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ====== Form Tambah ====== */}
      <div className="rounded-2xl border bg-white p-4 shadow-sm bg-neutral-200">
              ...
              {Array.isArray(userList) && userList.length > 0 ? (
                userList.map((user) => (
                  <tr key={user.id}  className="border-b last:border-0">
                    ...
                    <td className="px-3 py-2 text-left">{user.roles}</td>
